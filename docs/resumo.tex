...
As duas figuras a seguir mostram os dados de treinamento. Na primeira, \figureref{samples_by_cluster}, os dados são divididos nos clusters gerados pelas gaussianas definidas anteriormente. Na segunda, \figureref{samples_by_class}, são dividos em duas classes.

\begin{figure}[ht]
    \centering
    \includegraphics[scale=0.3]{samples_by_cluster}
    \caption{Classes 1a (verde), 1b (amarelo) e 2 (azul).}
    \label{fig:samples_by_cluster}
\end{figure}

\begin{figure}[ht]
    \centering
    \includegraphics[scale=0.3]{samples_by_class}
    \caption{Classes 1 (vermelho) e 2 (azul).}
    \label{fig:samples_by_class}
\end{figure}
